...
void clawStateChange(ClawState state)
{
    switch (state)
    {
    case OPEN:
        motorMove2PosFast(&motor4, 6400, 10000);
        break;
    case CLOSE:
        motorMove2PosFast(&motor4, 6400, 0);
        break;
    case SQUARE:
        motorMove2PosFast(&motor4, 6400, 2000);
        break;
    case CIRCLE:
        motorMove2PosFast(&motor4, 6400, 1500);
        break;
    default:
        break;
    }
...
void moveMotor(uint8_t motor, int32_t v1, float accTime, float decTime, int32_t step)
{
    switch (motor)
    {
    case 1:
...
void waitMotor(uint8_t motor)
{
    switch (motor)
    {
        case 1:
            WAIT(motor1.state == IDLE);
            break;
        case 2:
            WAIT(motor2.state == IDLE);
            break;
        case 3:
            WAIT(motor3.state == IDLE);
            break;
        case 4:
            WAIT(motor4.state == IDLE);
            break;
        case 5:
            WAIT(motor5.state == IDLE);
            break;
    }
}

void move2PosMotor(uint8_t motor, int32_t v1, float accTime, float decTime, int32_t pos)
{
    switch (motor)
    {
    case 1:
...
void motorMove2Pos(MotorControl *motor, int32_t v1, float accTime, float decTime, int32_t pos)
{
    ...
    motorMove(motor, v1, accTime, decTime, step);
}

void motorMove2PosFast(MotorControl *motor, int32_t v1, int32_t pos)
{
    int32_t step = pos - motor->actPos;
 ...
 * @param motor
 */
void motorPosCal(MotorControl *motor)
{
